...
const GCSCalculator: React.FC<GCSCalculatorProps> = ({ value, onChange }) => {
  ...
  const updateScore = (type: 'eyes' | 'verbal' | 'motor', score: number) => {
    ...
    // Only calculate total if all components are present
    if (newScore.eyes && newScore.verbal && newScore.motor) {
        newScore.total = newScore.eyes + newScore.verbal + newScore.motor;
    } else {
        newScore.total = undefined;
    }
  ...
  return (
    <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl space-y-4">
       <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-2">
           ...
           <div className="text-xl font-bold bg-white dark:bg-slate-800 px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white">
               {localScore.total || '--'}<span className="text-xs text-slate-400 dark:text-slate-500">/15</span>
           </div>
       </div>

       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
           <div className="flex flex-col gap-1">
               <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Eyes (4)</span>
           ...
           <div className="flex flex-col gap-1">
               <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Verbal (5)</span>
           ...
           <div className="flex flex-col gap-1">
               <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Motor (6)</span>
