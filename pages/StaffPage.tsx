...
const StaffPage = () => {
  ...
  const handleDeleteDoc = async (documentToDelete: ComplianceDoc) => {
      if (!editingUser || !editingUser.compliance) return;
      if (!confirm(`Are you sure you want to delete ${documentToDelete.name} for this user?`)) return;

      try {
          const newCompliance = editingUser.compliance.filter(d => d.id !== documentToDelete.id);
